...
function npm_deps_is_chenged(){
  let last_2_commit = shell.exec(`git log --pretty=format:'%H' -2`).stdout.split(os.EOL)
  let [ current_commit_package_json, prev_commit_package_json ] = last_2_commit.map(get_npm_deps_from_commit).map(c=>JSON.stringify(c))
  return current_commit_package_json !== prev_commit_package_json
}

/**
 * @typedef {{ build_cmd:string, dist:string, 'install-only':boolean }} Argv
 */
module.exports = {
