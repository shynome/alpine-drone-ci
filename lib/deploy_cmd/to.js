...
module.exports = {
  ...
  builder(yargs){
    return yargs
    .option('host',{ require:true, type:'string' })
  },
  /**@param {Argv} argv */
  handler(argv){
    let [ build_pack_file, deploy_dir, has_packed, deleted_files, current_commit ] = [ 'build_pack_file', 'deploy_dir', 'has_packed', 'deleted_files', 'current_commit' ].map(env.get)

    let tmp_file = `/tmp/${env.get('current_commit')}.tgz`
